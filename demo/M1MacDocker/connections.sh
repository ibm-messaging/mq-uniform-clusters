...
if [ ! -x "$(command -v runmqsc)" ]; then
  ...
  for (( i=0; i<100000; ++i)); do
    ...
    `(echo "$password"; echo "dis conn(*) where(appltag eq 'amqsphac') conntag") | runmqsc -c -u admin $qmName 2> /dev/null 1> /tmp/showConn.$qmName.amqsphac`
    `(echo "$password"; echo "dis conn(*) where(appltag eq 'amqsghac') conntag") | runmqsc -c -u admin $qmName 2> /dev/null 1> /tmp/showConn.$qmName.amqsghac`
